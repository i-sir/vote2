...
class ActivityVoteInit extends Base
{
    ...
    public function get_join_list($where = [], $params = [])
    {
        ...
        $result = $ActivityVoteModel
            ->alias('v')
            ->join('activity_log l', 'v.log_id = l.id')
            ->where($where)
            ->group('v.log_id')
            ->order('v.id desc')
            ->field('l.*')
            ->paginate(["list_rows" => $params["page_size"] ?? $this->PageSize, "query" => $params])
            ->each(function ($item, $key) use ($params, $ActivityLogInit) {

                /** 处理公共数据 **/
                $item = $ActivityLogInit->common_item($item, $params);
     ...
     * @return false|mixed
     */
    public function get_join_list2($where = [], $params = [])
    {
        $ActivityVoteModel = new \initmodel\ActivityVoteModel(); //投票记录  (ps:InitModel)
        ...
        $result = $ActivityVoteModel
            ...
            ->group('v.user_id')
            ->order('v.id desc')
            ->field('m.id,m.nickname,m.avatar')
            ->limit(20)
            ->select()
            ->each(function ($item, $key) use ($params) {
                ...
                $item['nickname'] = $this->maskNickname($item['nickname']);
    ...
    public function edit_post($params, $where = [])
    {
        ...
        if (!empty($where)) {
            ...
            $result                = $ActivityVoteModel->where($where)->strict(false)->update($params);
            //if ($result) $result = $item["id"];
        } elseif (!empty($params["id"])) {
            //如传入id,根据id编辑数据
            $params["update_time"] = time();
            $result                = $ActivityVoteModel->where("id", "=", $params["id"])->strict(false)->update($params);
            //if($result) $result = $item["id"];
        } else {
            //无更新条件则添加数据
            $params["create_time"] = time();
            $result                = $ActivityVoteModel->strict(false)->insert($params, true);
        }
    ...
    public function edit_post_two($params, $where = [])
    {
        ...
        if (!empty($where)) {
            ...
            //无更新条件则添加数据
            $params["create_time"] = time();
            $result                = $ActivityVoteModel->strict(false)->insert($params);
        }
    ...
    public function delete_post($id, $type = 1, $params = [])
    {
        ...
        if ($type == 1) $result = $ActivityVoteModel->destroy($id);//软删除 数据表字段必须有delete_time
        if ($type == 2) $result = $ActivityVoteModel->destroy($id, true);//真实删除
    ...
    public function batch_post($id, $params = [])
    {
        ...
        $params["update_time"] = time();
        $result                = $ActivityVoteModel->where($where)->strict(false)->update($params);//修改状态
    ...
    public function list_order_post($list_order, $params = [])
    {
        ...
        foreach ($list_order as $k => $v) {
            $where   = [];
            $where[] = ["id", "=", $k];
            $result  = $ActivityVoteModel->where($where)->strict(false)->update(["list_order" => $v, "update_time" => time()]);//排序
        }
    ...
    public function export_excel($where = [], $params = [])
    {
        ...
        $result = $ActivityVoteInit->get_list($where, $params);
        ...
        $Excel->excelExports($result, $headArrValue, ["fileName" => "投票记录"]);
    }
