...
class ActivityInit extends Base
{
    ...
    public function common_item($item = [], $params = [])
    {
        ...
        //投票剩余秒数
        $item['surplus_time'] = $item['attend_end_time'] - time();

        /** 处理数据 **/
        if ($this->InterfaceType == 'api') {
            /** api处理文件 **/
            if ($item['image']) $item['image'] = cmf_get_asset_url($item['image']);//图片介绍
    ...
    public function edit_post($params, $where = [])
    {
        ...
        //处理时间格式
        if ($params['attend_begin_time'] && is_string($params['attend_begin_time'])) $params['attend_begin_time'] = strtotime($params['attend_begin_time']);//报名开始时间
        if ($params['attend_end_time'] && is_string($params['attend_end_time'])) $params['attend_end_time'] = strtotime($params['attend_end_time']);//报名结束时间
        if ($params['begin_time'] && is_string($params['begin_time'])) $params['begin_time'] = (strlen($params['begin_time']) <= 10) ? strtotime($params['begin_time'] . ' 00:00:00') : strtotime($params['begin_time']);//活动开始时间
        if ($params['end_time'] && is_string($params['end_time'])) $params['end_time'] = (strlen($params['end_time']) <= 10) ? strtotime($params['end_time'] . ' 23:59:59') : strtotime($params['end_time']);//活动结束时间
    ...
    public function export_excel($where = [], $params = [])
    {
        ...
        $Excel->excelExports($result, $headArrValue, ["fileName" => "活动管理"]);
    }
