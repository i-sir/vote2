 ...
 *     "version"             =>"1.0",
 *     "use"                 => new \app\admin\controller\SignController();
 * )
 */
...
class SignController extends AdminBaseController
{
    ...
    public function index()
    {
        ...
        /** 导出数据 **/
        if ($params["is_export"]) $SignInit->export_excel($where, $params);


        /** 查询数据 **/
        $result = $SignInit->get_list_paginate($where, $params);
    ...
    public function add_post()
    {
        ...
        /** 检测参数信息 **/
        $validateResult = $this->validate($params, 'Sign');
        if ($validateResult !== true) $this->error($validateResult);


        /** 插入数据 **/
        $result = $SignInit->admin_edit_post($params);
    ...
    public function find()
    {
        ...
        $result                  = $SignInit->get_find($where, $params);
    ...
    public function edit_post()
    {
        ...
        /** 提交数据 **/
        $result = $SignInit->admin_edit_post($params, $where);
        if (empty($result)) $this->error("失败请重试");
    ...
    public function edit_post_two()
    {
        ...
        /** 提交数据 **/
        $result = $SignInit->edit_post_two($params, $where);
        if (empty($result)) $this->error("失败请重试");
    ...
    public function delete()
    {
        ...
        /** 删除数据 **/
        $result = $SignInit->delete_post($id);
        if (empty($result)) $this->error("失败请重试");
    ...
    public function batch_post()
    {
        ...
        //提交编辑
        $result = $SignInit->batch_post($id, $params);
        if (empty($result)) $this->error("失败请重试");
    ...
    public function list_order_post()
    {
        ...
        //提交更新
        $result = $SignInit->list_order_post($params);
        if (empty($result)) $this->error("失败请重试");
