 ...
 *     "version"             =>"1.0",
 *     "use"                 => new \app\admin\controller\VideoController();
 * )
 */
...
class VideoController extends AdminBaseController
{
    ...
    {
        $VideoClassInit = new \init\VideoClassInit();//分类管理     (ps:InitController)
        $class_map      = [];
        $class_map[]    = ['id', '<>', 0];
        $class_map[]    = ['is_show', '=', 1];
        $this->assign('class_list', $VideoClassInit->get_list($class_map));
    ...
    public function index()
    {
        ...
        /** 导出数据 **/
        if ($params["is_export"]) $VideoInit->export_excel($where, $params);


        /** 查询数据 **/
        $result = $VideoInit->get_list_paginate($where, $params);
    ...
    public function add_post()
    {
        ...
        /** 检测参数信息 **/
        $validateResult = $this->validate($params, 'Video');
        if ($validateResult !== true) $this->error($validateResult);


        /** 插入数据 **/
        $result = $VideoInit->admin_edit_post($params);
    ...
    public function edit_post()
    {
        ...
        /** 提交数据 **/
        $result = $VideoInit->admin_edit_post($params, $where);
        if (empty($result)) $this->error("失败请重试");
    ...
    public function edit_post_two()
    {
        ...
        /** 提交数据 **/
        $result = $VideoInit->edit_post_two($params, $where);
        if (empty($result)) $this->error("失败请重试");
    ...
    public function delete()
    {
        ...
        /** 删除数据 **/
        $result = $VideoInit->delete_post($id);
        if (empty($result)) $this->error("失败请重试");
    ...
    public function batch_post()
    {
        ...
        //提交编辑
        $result = $VideoInit->batch_post($id, $params);
        if (empty($result)) $this->error("失败请重试");
    ...
    public function list_order_post()
    {
        ...
        //提交更新
        $result = $VideoInit->list_order_post($params);
        if (empty($result)) $this->error("失败请重试");
